...
def main():
    parser = argparse.ArgumentParser(
        description="Verify rpi barcode decoding against MSW session."
    )
    parser.add_argument("--session", required=True, help="MSW session directory")
    parser.add_argument(
        "--ttl_in",
        default=None,
        help="Path to ttl_in.npz (auto-discovered if omitted)",
    )
    ...
    assert ttl_in is not None and ttl_in.exists(), (
        "ttl_in.npz not found. Use --ttl_in to specify path."
    )
    ...
    if result["wall_time_errors_ms"]:
        ...
        print(
            f"Wall time error (ms): mean={result['wall_time_error_mean_ms']:.1f}  "
        ...
        print(
            "  (positive = rpi edge after MSW wall_time, expected from NTP + pigpio latency)"
        )
    ...
    if result["unmatched_rpi_values"]:
        ...
        print(
            f"  {result['unmatched_rpi_values'][:10]}{'...' if len(result['unmatched_rpi_values']) > 10 else ''}"
        )
